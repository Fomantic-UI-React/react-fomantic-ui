...
/**
 * Reads a `.d.ts` file out of src as text.
 *
 * The Karma harness pulled these through webpack's `raw-loader`; there is no
 * bundler in the test path any more, so read from disk directly.
 */
export const requireTs = (tsPath) => {
  try {
    return fs.readFileSync(path.join(srcDir, tsPath), 'utf8')
  } catch (e) {
    return false
  }
}
